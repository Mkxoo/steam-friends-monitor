...
function SetSteamIDFromHTML(html) {
...
//从本地获取当前登录ID的存储的好友列表
async function GetFriendsListFromLocal() {
    if (currentID.length == id64len) {
        let nm = "fl" + currentID;
        let vv = await GetLocalValue(nm, []);
        return vv;
    }
    return [];
}
//从本地获取当前登录ID的存储的好友列表
async function SaveFriendsListToLocal(list) {
    if (currentID.length == id64len) {
        let nm = "fl" + currentID;
...
async function GetFriendsListFromOnline() {
...
// 更新一个日志到存储里
async function UpdateFriendsChangeLog(log) {
    if (currentID.length == id64len) {
        let nm = "cs" + currentID;
        let olds = await GetLocalValue(nm, []);
        olds.push(log);
        return SaveLocalValue(nm, olds);
    }
}
//一次完整的steam好友检查
async function DoOnceFriendsListCheck(test = false) {
...
function TryGetSteamID64ByURL(u) {
...
class SteamChatLogDownloader {
    ...
    constructor(title, stops = 0) {
        this.title = "";
        this.stops = 0;
        this.passedPages = 0;
        this.oldestTime = (new Date).getTime();
        this.downloadedlogs = [];
        this.waitURLs = [];
        this.stat = SteamChatLogDownloaderStat.ready;
        this.oncomplete = function () { };
        this.errorMessage = "";
        this.usedSeconds = new Map();
        this.usedMS = new Map();
        this.stops = stops;
        this.title = title;
    }
    PushToDownloaded(array) {
        let me = this;
    ...
    async ReplaceAllURLtoID64() {
        let me = this;
        let urls = CloneArray(me.waitURLs);
        let goodids = new Map();
        while (urls.length > 0) {
            let lefts = [];
...
async function AutoRemindBackupChatLog() {
    if (currentID.length == id64len) {
        ...
        if (autoremindchatlog > 0) {
            let nows = new Date;
            let nowtime = nows.getTime();
            let lastdownload = await GetLocalValue("downloadtime" + currentID, 0);
...
browser.runtime.onMessage.addListener(async function (m, sender) {
    ...
    if (showMessageLog) {
        console.log("接到来自其他网页的消息输入", m, sender);
    }
    let tabid = tab.id;
    if (m == null) {
        if (showMessageLog) {
            console.error("这不可思议，发了个null到后台");
        }
        return;
    }
    if (typeof (m) == "object") {
...
async function DownloadAllAndUploadAll(retry, lastError = "") {
    ...
    let nextupload = await GetLocalValue("nextupload" + currentID, 0);
...
async function BackgroundDownloadAndUpload(logs) {
    ...
    let uploaded = await GetLocalValue("githubuploaded" + currentID, []);
